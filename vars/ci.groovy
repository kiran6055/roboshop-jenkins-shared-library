def call() {
  try {
    node('JenkinsAgent') {
      stage('checkout') {
        cleanWs()
        git branch: 'main', url: "https://github.com/kirankumar7163/cart"
        sh 'env'
      }
      stage('compile/build') {
          common.compile()
      ...
      stage('qualitycontrol') {
          ...
          wrap([$class: 'MaskPasswordsBuildWrapper', varPasswordPairs: [[password: "${SONAR_PASS}", var: 'SECRET']]]) {
              sh "sonar-scanner -Dsonar.host.url=http://172.31.2.94:9000 -Dsonar.login=${SONAR_USER} -Dsonar.password=${SONAR_PASS} -Dsonar.projectKey=${component}"
          }
      }
